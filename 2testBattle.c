...
int play_0(int ID,int n,int SC[2] , int *H)
{
    int hyou[4]={SCT[0][0],SCT[0][1],SCT[1][0],SCT[1][1]};
    int i=0,max=0;
    ...
    if(ID==0){      //ホーム戦アウェイ戦両対応
        if(SCT[0][0]<=SCT[0][1] && SCT[0][1]<=SCT[1][0] && SCT[1][0]<=SCT[1][1]){
            return 1;
        }else if(SCT[0][0]<=SCT[0][1] && SCT[0][1]<=SCT[1][0] && SCT[1][0]<=SCT[1][1]){
            return 0;
        }
    }else{
        }
